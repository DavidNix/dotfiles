...
This skill assumes the user is already authenticated with glab. If commands fail with authentication errors, direct the user to run:
```bash
glab auth login
```
...
1. **Check auth status** if commands fail: `glab auth status`
