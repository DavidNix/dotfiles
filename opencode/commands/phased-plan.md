...
- Before coding, record `[~] IN PROGRESS` using the backend's protocol: the designated builder includes the file update in its first commit (RED for TDD work), while the orchestrator edits a GitHub phase issue before delegation.
- The orchestrator decides whether each implementation or fix work unit needs staged TDD and states the decision in the handoff, following `/phased-build`. TDD work uses two assignments, each in a fresh builder session without `task_id`: RED writes tests, proves the expected failures, commits, and returns evidence; GREEN receives the accepted RED commit, reproduces its failures before editing, implements, verifies, and commits. Each builder follows only its assigned stage's self-contained contract. Do not load or require `ai-tdd` for this workflow. Non-TDD work uses one implementation, verification, and commit assignment. Builders fix LSP diagnostics in changed files. For mixed phases, assign shared integration checks to the builder whose work completes the dependency.
- The orchestrator confirms RED evidence within the RED todo before launching a fresh GREEN builder, without a separate confirmation todo or rerunning phase checks. Resume a session only to finish or correct that same assignment. Review and final-gate fixes follow the same fresh-session RED/GREEN contract. Do not repeat unaffected successful checks.
...
During implementation, the orchestrator maintains one flat todo list using `[agent-type] Phase N: Short action` and `[agent-type] Final: Short action`, following `/phased-build`. Each TDD work unit has two consecutive builder items: write RED tests and commit; implement to GREEN, verify, and commit. Non-TDD work has one item. Evidence submission and confirmation stay within these items; review items remain separate. Expected behavioral failures complete RED successfully. Do not add separate verification, commit, handoff, or bookkeeping items per phase.
