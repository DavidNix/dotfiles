...
- Data-model-first within cohesive feature slices: generally include a data-model phase before the implementation phases that depend on it. Define and implement only the entities, fields, identifiers, relationships, constraints, ownership, lifecycle, indexes or query patterns, and migration/backfill/rollback needs required by that slice. Defer model additions needed only by later slices to a later data-model phase before their dependent implementation. Do not invent data-model work when no persisted or shared state changes.
...
For feature work, use this default sequence: define and implement the next cohesive slice's data model, then build and verify its dependent behavior in isolation. Repeat as needed for later slices rather than front-loading the entire project's data model. Wire the feature into production in the final phase, which owns production entrypoints, activation, and end-to-end verification. State each phase's production impact and how verification establishes that inactive work leaves existing behavior unchanged.
...
- The orchestrator inherits the invoking session's model and reasoning settings. It may split broad phases into focused, independently verifiable work units for low-reasoning builders without changing phase scope or creating new phase issues. Resolve design decisions and interfaces before delegation; keep tightly coupled changes and the full TDD cycle together. Simple phases stay as one unit, and reviews remain phase-level.
...
- Before coding, record `[~] IN PROGRESS` using the backend's protocol: the designated builder includes the file update in its implementation commit, while the orchestrator edits a GitHub phase issue before delegation.
- The orchestrator decides whether each implementation or fix work unit warrants TDD and states the decision in the handoff, following `/phased-build`. When warranted, explicitly instruct the builder to load `ai-tdd` and complete RED and GREEN in one session, then verify and create one atomic commit. Terraform, Ansible, other infrastructure work, Go `main()` functions and entrypoint wiring, documentation, and non-behavioral configuration changes use suitable validation instead. Builders fix LSP diagnostics in changed files. For mixed phases, apply TDD only to suitable application logic and assign shared integration checks to the builder whose work completes the dependency.
- Use a fresh builder session per work unit and resume it to finish or correct the same assignment. Confirm its commit and verification evidence, including RED/GREEN evidence when applicable, without rerunning phase checks. Review and final-gate fixes follow the same workflow. Do not require separate RED assignments, approval gates, or commits, and do not repeat unaffected successful checks.
- Preserve and record historical TDD-ordering deviations accurately, then continue verification and review under `/phased-build`'s recovery policy without a separate user-acceptance gate. Never describe post-hoc regression evidence as test-first RED.
- After verification passes, run one code review over each non-final phase's complete commit range. Builders fix blocking findings in one pass with targeted verification; record minor follow-ups under `/phased-build`'s deferral policy. Do not repeat the phase review. The last selected phase gets no separate phase review and remains active until the final gate, unless it was already complete before a scope reduction; preserve existing history in that case.
- After the last selected phase's implementation and checks finish, the orchestrator runs full verification. Then dispatch final code and security reviews in parallel against the same selected range. Wait for both, combine verification failures and blocking findings by responsible builder, and assign fixes with targeted checks. The orchestrator reruns failed and fix-affected checks before finalizing; do not repeat unaffected checks or reviews.
- The orchestrator marks a phase `[x] COMPLETE` only after verification passes and every review finding is fixed, accepted, invalidated, or durably deferred under `/phased-build`'s minor-follow-up policy. Store follow-ups in the repository-designated backlog or default to `FOLLOWUPS.md` in file mode and one standalone follow-up issue in GitHub mode. Keep them outside managed phases so they survive plan cleanup without blocking completion.
...
During implementation, the orchestrator maintains one flat todo list using `[agent-type] Phase N: <brief phase outcome>` and `[agent-type] Final: <check and project subject>`, following `/phased-build`. Derive concise titles from each phase's name and outcome; name the delivered behavior rather than RED/GREEN, verification, or commit steps. Review titles identify the same phase subject, never just `Review changes`. Distinguish work-unit slices while keeping the phase subject recognizable. Each work unit has one builder item covering implementation, verification, and commit. TDD and evidence confirmation stay within that item; review items remain separate. Do not add separate test-writing, verification, commit, handoff, or bookkeeping items per phase, or active todos for deferred follow-ups.
